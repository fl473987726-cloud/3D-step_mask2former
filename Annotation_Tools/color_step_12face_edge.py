# 为STEP文件的每个面和边根据几何属性分配指定的颜色，并输出12视角渲染图
# 输入step文件（没有染色的），输出step文件（有染色的）和12视角渲染图的png文件
import os
import glob
import json
import math
...
def render_12_views(face_render_items, edge_render_items, bounds, image_output_dir):
...
def process_step(input_file, output_file, render_output_dir=None):
    ...
    if render_output_dir is not None and face_render_items:
        render_12_views(face_render_items, edge_render_items, bounds, render_output_dir)

if __name__ == "__main__":
    # --- 替换为你要求的路径 ---
    input_dir = r"E:\aaaa-WUT\lw\ASCCAD\test_step\slot_test01"
    
    # 单独输出“面+边属性染色”结果，避免覆盖之前仅面染色的文件
    output_dir = r"E:\aaaa-WUT\lw\ASCCAD\test_step\slot_test01_face_edge_colored"
    render_dir = r"E:\aaaa-WUT\lw\ASCCAD\test_step\slot_test01_face_edge_colored_renders"
